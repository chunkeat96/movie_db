...
if [[ $build_device -eq 0 || $build_device -eq 2 ]]; then
  if [[ $build_flavor == "dev" ]]; then
    flutter build apk -t $build_targer_directory --build-name=$build_name --build-number=$build_number
  elif [[ $build_flavor == "prod" ]]; then
    ...
    if [[ $bundle_type == "apk" ]]; then
      ...
      flutter build appbundle -t $build_targer_directory --build-name=$build_name --build-number=$build_number
    fi
  fi
fi
